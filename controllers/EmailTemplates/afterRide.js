...
module.exports = function afterRide(ride) {
  ...
  const text = `Tucána | 
  Tenemos curiosidad por saber cómo fue el ride de ${ride.origin.placeName} a ${
  ...
  We are curious to know how was the ride from ${ride.origin.placeName} to ${
    ride.destination.placeName
  } (${dateFormat(
    ride.dateTimeOrigin,
    "dd/mm/yyyy"
  ...
  const html = emailHtmlTemplate({
    titleEN: `How was your ride from ${ride.origin.placeName} to ${
      ride.destination.placeName
    ...
    titleES: `¿Cómo fue tu viaje de ${ride.origin.placeName} a ${
      ride.destination.placeName
    } (${dateFormat(ride.dateTimeOrigin, "dd/mm/yyyy")})?`,
